...
class DiscussiveMode extends StatefulWidget {
  const DiscussiveMode({Key? key}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _DiscussiveModeState createState() => _DiscussiveModeState();
...
void clearMapLists(Map<int, List<String>> map) {
  map.forEach((key, value) {
    value.clear(); // Clear each list
  });
}

String clickabletopaction = '';
String coldplayaction(
  String currentclickabletop,
) {
...
class _DiscussiveModeState extends State<DiscussiveMode> {
  ...
  void initState() {
    ...
    shuffledIndexes =
        List.generate(cardDeckData.frontCard.length, (index) => index);
    shuffleActionCards();
  }
  ...
  void switchToNextPlayer() {
    setState(() {
      Globals.currentplayer =
          (Globals.currentplayer % Globals.numberOfPlayers) + 1;
    });
  }
  ...
  Widget build(BuildContext context) {
    final playerBarWidth =
        MediaQuery.of(context).size.width / Globals.numberOfPlayers;
                ...
                children: [
                  for (int i = 0; i < 25; i++)
                    if (i + currentIndex >= 0 &&
                      ...
                      LetterCard(
                        //bookmark
                        backcard: coldplay(_addWordToFilename(cardDeckData
                            .frontCard[shuffledIndexes[i + currentIndex]])),
                        frontCardAsset: cardDeckData
                            .frontCard[shuffledIndexes[i + currentIndex]],
                        backCardAsset: drums(_addWordToFilename(cardDeckData
                            .frontCard[shuffledIndexes[i + currentIndex]])),
                        rotation: (i == 0)
                            ? 0.0
                            : (i < 2)
                  ...
                  for (int i = 0; i < 25; i++)
                    if (i + actionCardIndex >= 0 &&
                      ...
                      ActionCard(
                        backcard: coldplayaction(_addDescToFilename(actionCards[
                            shuffledActionIndexes[i + actionCardIndex]])),
                        frontCardAsset: getactioncard(actionCards[
                            shuffledActionIndexes[i + actionCardIndex]]),
                        backCardAsset: _addDescToFilename(actionCards[
                            shuffledActionIndexes[i + actionCardIndex]]),
                        rotation: (i == 0)
                            ? 0.0
                            : (i < 2)
